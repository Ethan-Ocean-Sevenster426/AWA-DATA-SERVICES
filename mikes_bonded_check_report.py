#!/usr/bin/env python3
"""
AWA Data Services - Mike Bonded Check Report
============================================
Pulls the "Mike Bonded Check" saved search (receive consignments with Service Level
STC = "Customs clearance only" - i.e. bonded goods - that have packages in the
warehouse) from CargoWise TWD (WiseGrid) for the DOR branch, and writes a single
"Receive Consignment" sheet matching the master "Mikes bonded check .xlsx", then
(optionally) uploads to SharePoint via Microsoft Graph.

Saved search "Mike Bonded Check" (module IEntityInfo_IWhsItemReceiveConsignment):
  * SERVICELEVEL            Is 'STC'
  * HASPACKAGESINWAREHOUSE  Is Yes   (matches the master file's content - bonded
                                      goods physically in the warehouse)
  * RECEIVEGATEINTIME       (date range - left open / not constrained)
Set MBC_REQUIRE_IN_WAREHOUSE=false to drop the warehouse condition and pull every
STC consignment instead (matches the live grid as currently toggled).

Package columns are the SUM of package quantities (KP_PackageQty) by status.
Timezone: "Completion date" (actual) -> true-instant converted to display tz (UTC-6).
...
BRANCH_CODES   = [b.strip().upper() for b in _env("MBC_BRANCH_CODES", "DOR").split(",") if b.strip()]
DEPARTMENT_CODE= _env("CW_DEPARTMENT_CODE", "BRN").upper()
SERVICE_LEVEL  = _env("MBC_SERVICE_LEVEL", "STC")
REQUIRE_INWHS  = _env("MBC_REQUIRE_IN_WAREHOUSE", "true").lower() in ("1", "true", "yes")
DISPLAY_TZ     = int(_env("MBC_DISPLAY_TZ_OFFSET", _env("CC_DISPLAY_TZ_OFFSET", "-6")))

OUTPUT_DIR     = _env("OUTPUT_DIR", "./output")
OUTPUT_FILE    = _env("MBC_FILENAME", "Mikes bonded check .xlsx")
...
SP_FOLDER      = _env("MBC_SHAREPOINT_FOLDER", _env("RTU_SHAREPOINT_FOLDER", "RTU Report"))

# Exact (sentence-case) headers from the master file; Branch is the LAST column.
COLUMNS = ["Receive consignment ID", "Completion date", "Booking party", "RCN reference", "Consignor",
           "Consignee", "Number of packages", "BKD", "In warehouse", "Warehouse", "Service level", "Branch"]
DATE_COLS = {"Completion date"}
WIDTHS = {"Receive consignment ID": 22.6, "Completion date": 15.6, "Booking party": 45, "RCN reference": 14.9,
          "Consignor": 45, "Consignee": 45, "Number of packages": 18, "In warehouse": 12, "Warehouse": 30,
          "Service level": 24, "Branch": 8}
INWHS = {"ARV", "PUT", "PIC", "CTT", "STA", "FLO", "REC", "RCV"}

log = logging.getLogger("mikes_bonded_check_report")
...
class CargoWise:
    ...
    def pull_branch(self, branch_key):
        ...
        filt = f"WRC_RS_NKServiceLevel eq '{SERVICE_LEVEL}'"
        if REQUIRE_INWHS:
            inwhs = " or ".join(f"p/WPS_Status eq '{s}'" for s in sorted(INWHS))
            filt += f" and WhsItemPackageStates/any(p: {inwhs})"
        ...
        select = "WRC_JobID,WRC_ConsignmentID,WRC_CompleteTime,WRC_RS_NKServiceLevel"
        rows, skip, page = [], 0, 50
        while True:
...
def shape(branch, records, svc_map):
    ...
    for r in records:
        ...
        out.append({
            ...
            "Completion date": parse_dt_actual(r.get("WRC_CompleteTime")),
...
def main():
    ...
    log.info("Mike Bonded Check - branches=%s, service level=%s, require in-warehouse=%s",
             BRANCH_CODES, SERVICE_LEVEL, REQUIRE_INWHS)
