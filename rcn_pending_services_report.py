#!/usr/bin/env python3
"""
AWA Data Services - RCN Pending Services Report ("Services Pending")
===================================================================
Pulls the "RCN Pending Services (booked but not completed)" saved search (receive
consignments that have a service booked in the last 6 months and NO service yet
completed) from CargoWise TWD (WiseGrid) for both branches (DOR + CON), and writes
a single "Services Pending" sheet matching the master "Services Pending.xlsx", then
(optionally) uploads to SharePoint via Microsoft Graph.

Saved search "RCN Pending Services (booked but not completed)"
(module IEntityInfo_IWhsItemReceiveConsignment):
  * SERVICEBOOKEDDATE     WasInTheLast6Months -> a service booked in the last 6 months
  * SERVICECOMPLETEDDATE  HasNoDate           -> NO service has a completed date
Both reproduced exactly (validated: returns the same 12 CON rows the grid shows).
(This is the broader cousin of "Services Pending - Freight on-hand", which adds a
"packages in warehouse" condition and uses HasDate instead of the 6-month window.)
...
"""
import os, sys, json, re, logging, calendar, datetime
...
BRANCH_CODES   = [b.strip().upper() for b in _env("RPS_BRANCH_CODES", "DOR,CON").split(",") if b.strip()]
DEPARTMENT_CODE= _env("CW_DEPARTMENT_CODE", "BRN").upper()
BOOKED_MONTHS  = int(_env("RPS_BOOKED_MONTHS", "6"))            # "service booked in the last N months"
DISPLAY_TZ     = int(_env("RPS_DISPLAY_TZ_OFFSET", _env("CC_DISPLAY_TZ_OFFSET", "-6")))

OUTPUT_DIR     = _env("OUTPUT_DIR", "./output")
OUTPUT_FILE    = _env("RPS_FILENAME", "Services Pending.xlsx")
...
SP_FOLDER      = _env("RPS_SHAREPOINT_FOLDER", _env("RTU_SHAREPOINT_FOLDER", "RTU Report"))
...
log = logging.getLogger("rcn_pending_services_report")
...
class CargoWise:
    ...
    def pull_branch(self, branch_key, cutoff_iso):
        ...
        # A service booked in the last N months AND no service completed.
        filt = (f"AdditionalServices/any(s: s/ES_BookedDateTimeOffset ge {cutoff_iso}) and "
                f"not AdditionalServices/any(s: s/ES_CompletedDateTimeOffset ne null)")
...
def next_open_service(rec):
    """The next open (not completed) service, ordered by booked date then ES_ServiceId.
    NOTE: for consignments with MULTIPLE open services CargoWise's exact ordering is not
    exposed in the OData feed, so the pick may differ from the grid for those few rows;
    single-service consignments (the common case) are exact."""
...
def build_workbook(rows, path):
    ...
    wb = openpyxl.Workbook(); ws = wb.active; ws.title = "Services Pending"
    ws.append(COLUMNS)
    for r in rows:
    ...
    ws.freeze_panes = "A2"
    ws.add_table(Table(displayName="ServicesPending",
...
def main():
    ...
    cutoff = months_ago_iso(BOOKED_MONTHS)
    log.info("RCN Pending Services - branches=%s, service booked since %s, no service completed",
             BRANCH_CODES, cutoff)
